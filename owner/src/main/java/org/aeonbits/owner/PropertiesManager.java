...
import static org.aeonbits.owner.util.Util.asString;
import static org.aeonbits.owner.util.Util.eq;
...
class PropertiesManager implements Reloadable, Accessible, Mutable {
	 ...
	 * <p>
	 * This allows each key has its own decryptor. Reflection is slow.
	 */
	...
	PropertiesManager(Class<? extends Config> clazz, Properties properties, ScheduledExecutorService scheduler,
		...
		// We try to identify the DecryptorClass annotation, to assign the Decryptor to
		// this configuration.
		...
		// Reflection is slow, so we will cache all methods with EncryptedValue
		// annotation.
	...
	/**
	 * If method contains the EncryptedValue annotation it Decrypts the value with
	 * the associated {@link Decryptor}.
	 *
	 * @param method with the key definition.
	 * @param value  the value to decrypt when the method contains the
	 *               EncryptedValue annotation
	 * @return the <code>value</code> if the method doesn't contains the
	 *         EncryptedValue annotation or the
	 *         <code>result of decrypt the value</code> if it does.
	 */
	String decryptIfNecessary(Method method, String value) {
		// Value can't be null, it has been checked previously in
		// PropertiesInvocationHandler.resolveProperty
	...
	private static class PropertyChangeListenerWrapper implements TransactionalPropertyChangeListener, Serializable {
		...
		PropertyChangeListenerWrapper(String propertyName, PropertyChangeListener listener, boolean transactional) {
	...
	private void fireBeforePropertyChange(PropertyChangeEvent event)
			throws RollbackBatchException, RollbackOperationException {
